...
void eval(char *cmdline) 
{
  ...
  if(argv[0] == NULL){ //nothing in commandline
     return;
   }
 

  if(builtin_cmd(argv)){ //if its a built in function
    //do the thing
    //jobs
    //fg
    //bg
 ...
 *    it immediately.  
 * Should be quit, jobs, bg <job>, fg<job>
 */
int builtin_cmd(char **argv) 
{
 ...
 * Pauses foreground process, resumed with builtin fg and bg  
 */
void sigtstp_handler(int sig) 
{
